...
/*   Updated: 2017/12/15 23:12:36 by yvillepo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		read_input(char	*argv, t_mlx *mlx)
{
	...
	tabnbr = ft_strsplit(*ligne, ' ');
	mlx->max.y = len_tabstr(ligne);
	mlx->max.x = len_tabstr(tabnbr);
	free_tabstr(tabnbr);
	free(tabnbr);
	mlx->nbpoint = mlx->max.x * mlx->max.y;
	mlx->p3D = ft_memalloc(sizeof(*(mlx->p3D)) * mlx->nbpoint);
	//mlx->p = centrer_points(mlx);
